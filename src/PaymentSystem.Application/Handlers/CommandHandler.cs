...
{
    public abstract class CommandHandler<T, TId> where T : class, IAggregate, new()
    {
        ...
        protected async Task PerformAsync(Command<TId> cmd, Action<T> action) {
            ...
            if (cmd.Version != null && cmd.Version.Value > 0 && cmd.Version != aggregate.Version)
                Trace.TraceWarning($"The supplied aggregate version {cmd.Version} is different from the loaded aggregate version {aggregate.Version}.");
            action(aggregate);
