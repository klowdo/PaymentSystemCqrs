...
namespace PaymentSystem.ReadModel.Services
{
    public class ProjectionConsumer<T> where T : Event<T>
    {
        ...
        public ProjectionConsumer(object projection)
        {
            ...
            _allMethods = _projection.GetType().GetMethods().Where(m =>
                m.Name == nameof(INotificationHandler<Event>.Handle) && m.GetParameters().Length == 1);
            ...
            if (nonMatchingParameter != null)
                throw new Exception(
                    $"This ProjectionConsumer can only handle Consume methods consuming {typeof(T).Name} events. Type {nonMatchingParameter.ParameterType.FullName} is not valid!");
#endif
        }
